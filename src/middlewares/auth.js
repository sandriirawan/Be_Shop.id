...
const validateSellerRole = (req, res, next) => {
  if (req.payload.role === 'seller') {
    next(); 
  } else {
    res.status(403).json({ error: 'You do not have access as a seller' });
  }
};
const admin = (req, res, next) => {
  if (req.payload.role === 'admin') {
    ...
    res.status(403).json({ error: 'Only admin can access' });
  }
};


module.exports = {protect,validateSellerRole,admin}